...
SELECT  ID, COMPANY_ID, OBJECT, CONCAT('R$ ', FORMAT(CONTRACT_VALUE, 2, 'pt_BR')) AS CONTRACT_VALUE
FROM CONTRACTS
...
SELECT ID, CONTRACT_ID, CONCAT('R$ ', FORMAT(PAID_VALUE, 2, 'pt_BR')) AS PAID_VALUE, PAY_DATE
FROM PAYMENTS
WHERE PAID_VALUE > (SELECT AVG(PAID_VALUE) FROM PAYMENTS);
...
SELECT *
FROM COMPANIES
WHERE ID IN (SELECT COMPANY_ID FROM CONTRACTS WHERE CONTRACT_VALUE > 300000);
...
SELECT C.ID AS CONTRACT_NUMBER, CP.COMPANY_NAME, CP.CITY, C.OBJECT, CONCAT('R$ ', FORMAT(C.CONTRACT_VALUE, 2, 'pt_BR')) AS CONTRACT_VALUE
FROM CONTRACTS C
INNER JOIN COMPANIES CP ON CP.ID = C.COMPANY_ID
WHERE EXISTS(SELECT * FROM PAYMENTS P WHERE P.CONTRACT_ID = C.ID)
...
FROM CONTRACTS C
INNER JOIN COMPANIES CP ON CP.ID = C.COMPANY_ID
WHERE C.ID IN (SELECT ID FROM CONTRACTS ORDER BY CONTRACT_VALUE DESC);

SELECT * FROM CONTRACTS
ORDER BY CONTRACT_VALUE DESC;

SELECT * FROM CONTRACTS ORDER BY CONTRACT_VALUE DESC LIMIT 3;

SELECT 
    C.ID AS CONTRACT_NUMBER, CP.COMPANY_NAME, CP.CITY, C.OBJECT, CONCAT('R$ ', FORMAT(C.CONTRACT_VALUE, 2, 'pt_BR')) AS CONTRACT_VALUE
FROM CONTRACTS C
INNER JOIN COMPANIES CP ON CP.ID = C.COMPANY_ID
